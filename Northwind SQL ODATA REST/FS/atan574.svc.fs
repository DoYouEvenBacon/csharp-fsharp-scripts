...
and [<AllowNullLiteral(); DataServiceKey("OrderID")>] 
  Order () = class  // public
    ...
    member val CustomerID: string = null with get, set 
    member val EmployeeID: Nullable<int> = Nullable<int>() with get, set
        ...
        _Orders <- XElement
            ...
            .Select(fun x -> 
                new Order (
                    ...
                    CustomerID = x.Element(xn "CustomerID").Value,
                    EmployeeID = (if String.IsNullOrEmpty(x.Element(xn "EmployeeID").Value) then Nullable<int>() else Nullable (Int32.Parse (x.Element(xn "EmployeeID").Value))),
                    ...
                    ShippedDate = (if not(x.Elements(xn "ShippedDate").Any()) || String.IsNullOrEmpty(x.Element(xn "ShippedDate").Value) then Nullable<DateTime>() else Nullable (DateTime.Parse (x.Element(xn "ShippedDate").Value))),
                    Freight = (if not(x.Elements(xn "Freight").Any()) || String.IsNullOrEmpty(x.Element(xn "Freight").Value) then Nullable<Decimal>() else Nullable (Decimal.Parse (x.Element(xn "Freight").Value))),
